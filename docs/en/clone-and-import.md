...
A project directory named `java-import-class-with-same-name` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-import-class-with-same-name` directory which is created just now.
